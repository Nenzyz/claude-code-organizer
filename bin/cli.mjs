...
if (isBackupMode) {
  // CLI backup mode: npx @mcpware/cross-code-organizer --backup <command> [args]
  ...
  if (subCmd === 'init') {
    ...
    if (remoteUrl) {
      ...
      console.log(`  ℹ No remote set. Add one with: npx @mcpware/cross-code-organizer --backup init <git-url>`);
    }
    ...
    if (!(await isGitRepo(BACKUP_DIR))) {
      console.error(`  ✗ Backup repo not initialized. Run: npx @mcpware/cross-code-organizer --backup init`);
      process.exit(1);
    }
    ...
    console.log(`
  Usage: npx @mcpware/cross-code-organizer --backup <command>
